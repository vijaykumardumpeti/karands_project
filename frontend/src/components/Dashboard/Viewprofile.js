...
function Viewprofile(props) {
    ...
    if (!params.userId) {
      ...
      axios.get(`${process.env.REACT_APP_IP_ADDRESS}/karands/users/${localStorage.getItem('id')}`)
        ...
        .then(res => {
          ...
          localStorage.setItem('fullName', roles.fullName);
      ...
      axios.get(`${process.env.REACT_APP_IP_ADDRESS}/karands/users/verification/${localStorage.getItem('id')}`)
        .then(res => setVerification(res.data))
        .catch(error => console.log(error))
    }
  }, [])
  ...
  function CalculateRating(details) {
    ...
    if (details.education) {
      ...
      const finalEducationScore = Math.min(educationScore, 2);
        ...
        <div className="row flex-nowrap" style={{ width: "100%", height: "100%" }}>
          ...
          <div className="col container" style={{ maxWidth: "80%" }}>
            ...
            <Fragment>
              ...
              <div className="container-lg container-xl p-3"
                    ...
                    <div className="row">
                      <div className="col-lg-4 col-xl-4">
                        ...
                        <div className="card-box text-center">
                          ...
                          <h4 className="mb-0 user-profile-name">{details.fullName ? details.fullName : details.name}</h4>
                          <p className="text-muted">{details.designation}</p>

                          <p className="text-muted user-profile-company-details">{experience[0]?.["companyName"]}</p>

                          <h6 className='text-muted user-profile-company-details'>Role: {details.role ? details.role : ""}</h6>

                          <p className="text-muted user-profile-company-details">{experience[0]?.["locationOfCompany"]}</p>

                          <LoaderBar value={userrating} /> {userrating}/10
...

                          {
                            ...
                            params.userId && params.userId != localStorage.getItem('id') ? (
                              <button type="button" className="btn btn-danger btn-xs waves-effect mb-2 waves-light" onClick={(e) => { handleclickdiv(e, { "_id": params.userId, "name": details.fullName ? details.fullName : details.name }) }}>Message</button>
                            ) : ""
                          }
                          ...
                          <div className="text-left mt-3">
                            <h4 className="font-13 text-uppercase m-2 " style={{ fontFamily: "Roboto" }}>About Me:</h4>
                              ...
                              <div className="tab-content" id="nav-tabContent">
                                <div className="tab-pane fade show active " id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                                  <div className="row">
                                    ...
                                    <div className="col-6" style={{ overflow: "scroll", overflowY: "hidden" }}>
                                      <h6 className="d-flex text-start fs-13 fw-normal" style={{ marginTop: "15px" }}>
                                        Link: <a href={`https://karandszone.com/sharedprofile/${localStorage.getItem("id")}`} target="_blank" rel="noopener noreferrer">{`https://karandszone.com/sharedprofile/${localStorage.getItem("id")}`}</a>
                                      </h6>
                                    </div>
                                          ...
                                          experience.map((exp, index) => {
                                            return <Fragment>
                                              <ul style={{ listStyle: "none" }} >
                                                ...
                                                <p className="d-flex text-start">Company Name:-{exp.companyName} </p>
                                                <p className="d-flex text-start mt-2">
                                                  Status:-<p style={{ color: exp.status == "Verified" ? "green" : exp.status == "Ongoing" ? "orange" : "red" }}>{exp.status}</p>
                                                </p>
                                                  ...
                                                  {exp.skills.map((each, index) => {
                                                    return <Fragment>
                                                      <div style={{ display: "flex", marginRight: "10px" }} >
                                                        <span className='badge bg-secondary mt-1 mb-1'>{each.value}</span>
                                                      </div>
                                                    </Fragment>
                                                  })}
                                                </p>
                                      ...
                                      <ul className="list-unstyled mb-1-9">
                                        ...
                                        {education.map((detail, index) => {
                                          return <li >
                                            ...
                                            <p className="d-flex text-start mt-2">
                                              Status:-<p style={{ color: detail.status == "Verified" ? "green" : detail.status == "Ongoing" ? "orange" : "red" }}>{detail.status}</p>
                                            </p>
