...
export default function Mentoring({ handleCityChange,handleDesignationChange,handleFunctionalAreaChange,handleInputChange, previous, next,AllCities,AllIndustry,AllFunctionalArea,AllSkills,AllDesignation }) {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_IP_ADDRESS}/karands/relation/${localStorage.getItem(
          "email"
        )}`
      )
      .then((res) => {
        ...
        if (someDetails != null && someDetails.surveyType==='Mentoring others') {
          setDetails(prevDetails => ({
            ...prevDetails,
            location: someDetails.location,
            about: someDetails.about,
            professionalExperience: someDetails.professionalExperience,
          }));
        setSignleLocation(someDetails.location)
  ...
  function connectionCheck() {
    ...
    axios
      .post(
        `${process.env.REACT_APP_IP_ADDRESS}/karands/relation/mentoring`,

        {
          ...
          about: details.about,
          professionalExperience: details.professionalExperience,
          email: localStorage.getItem("email"),
        }
      )
      .then((res) => {
  ...
  return (
    <div  className="objectSurveyPageContainerToHold">
     ...
     <p style={{ backgroundColor: "rgb(3, 104, 104)", color: "white" }} className="p-2">
          {" "}
          <b>Looking For Mentoring others</b>
        </p>
 <Select
 styles={customStyles}
...
<div className=" mt-3">
            <input
              type="text"
              value={details.professionalExperience}
              {...register("experience",{
          ...
          <span className="errorsInreg">{errors.experience&& errors.experience.message}</span>
 <Select
 styles={customStyles}
            options={AllCities}
            isSearchable={true}
